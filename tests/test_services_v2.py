...
class TestServicesV2:
    ...
    def test_loads_all_services(self):
        ...
        for service_id, schema in schemas.items():
            ...
            ), f"Service {service_id} dependencies not a list"

            # Every service should have an enabled field
            enabled_fields = [f for f in schema.fields if f.key == "enabled"]
            assert (
                len(enabled_fields) == 1
            ), f"Service {service_id} should have exactly one 'enabled' field, found {len(enabled_fields)}"

            enabled_field = enabled_fields[0]
            assert (
                enabled_field.type == "boolean"
            ), f"Service {service_id} enabled field should be boolean"
    ...
    def test_grafana_depends_on_prometheus(self):
        ...
        if "grafana" in schemas and "prometheus" in schemas:
            ...
            assert (
                "prometheus" in grafana_schema.dependencies
    ...
    def test_service_categories(self):
        ...
        for service_id, expected_category in expected_categories.items():
            if service_id in schemas:
                ...
                ), f"Service {service_id} should be in category '{expected_category}', got '{actual_category}'"
    ...
    def test_service_ids_valid_format(self):
        ...
        for service_id in schemas.keys():
            ...
            assert service_id.replace(
                "_", ""
            ).isalnum(), (
                f"Service ID '{service_id}' should only contain alphanumeric and underscores"
            )
            assert not service_id.startswith(
                "_"
            ), f"Service ID '{service_id}' should not start with underscore"
            assert not service_id.endswith(
                "_"
            ), f"Service ID '{service_id}' should not end with underscore"
    ...
    def test_profile_defaults_exist(self):
        ...
        for service_id, schema in schemas.items():
            if schema.defaults:
                ...
                assert (
                    schema.defaults.dev is not None or schema.defaults.prod is not None
                ...
                if has_enabled_field and schema.defaults.dev:
                    assert (
                        "enabled" in schema.defaults.dev
                    ), f"Service {service_id} dev defaults should specify 'enabled'"
                if has_enabled_field and schema.defaults.prod:
                    assert (
                        "enabled" in schema.defaults.prod
                    ), f"Service {service_id} prod defaults should specify 'enabled'"
