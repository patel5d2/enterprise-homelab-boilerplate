    ...
    if config_path.exists() and not non_interactive:
        try:
            ...
            if existing_config and not existing_config.get('version'):
                ...
                if Confirm.ask("Would you like to migrate to the new configuration format?"):
                    console.print("[blue]Migration feature will be available soon. Please backup your config and create a new one.[/blue]")
                    return
        except Exception:
...
def _show_next_steps_v2(config: dict) -> None:
    ...
    console.print(f"[dim]💡 Edit configuration anytime with: labctl config --edit[/dim]")
