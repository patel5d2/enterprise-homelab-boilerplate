...
from .prompter import ask_field, ask_custom_environment_variables, display_field_summary
...
def select_profile() -> str:
    ...
    console.print("• [cyan]Development[/cyan] - Staging certificates, debug logging, lower resource usage")
    console.print("• [cyan]Production[/cyan] - Production certificates, optimized settings, higher security")
...
def display_service_categories(schemas: Dict[str, ServiceSchema]) -> None:
    ...
    for category, service_ids in categories.items():
        ...
        for service_id in sorted(service_ids):
            ...
            deps_text = f" [dim](depends on: {', '.join(schema.dependencies)})[/dim]" if schema.dependencies else ""
            maturity_badge = ""
            ...
            console.print(f"  • [white]{schema.name}[/white]{maturity_badge} - {schema.description}{deps_text}")
...
def select_services(schemas: Dict[str, ServiceSchema]) -> Set[str]:
    ...
    while True:
        try:
            ...
            if invalid_services:
                console.print(f"[red]Error: Unknown services: {', '.join(invalid_services)}[/red]")
                console.print(f"[dim]Available: {', '.join(sorted(available_ids))}[/dim]")
                continue
        ...
        for key, value in config.items():
            ...
            if 'password' in key.lower() or 'token' in key.lower() or 'secret' in key.lower():
                ...
                if isinstance(value, bool):
                    ...
                    key_settings.append(f"{key}: {', '.join(value) if value else 'none'}")
                elif value:
...
def confirm_configuration(session: WizardSession) -> bool:
    ...
    while Confirm.ask("Edit any service configuration?", default=False, console=console):
        ...
        if service_input in session.schemas:
            ...
            session.service_configs[service_input] = configure_service(service_input, schema, session)
        else:
...
def run_wizard(schemas_dir: Path, profile: Optional[str] = None) -> Optional[WizardSession]:
    """
