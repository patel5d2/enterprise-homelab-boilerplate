...
class ConditionalExpressionEvaluator:
    ...
    def evaluate(self, expression: str) -> bool:
        ...
        try:
            ...
            console.print(f"[yellow]Warning: Could not parse expression '{expression}', defaulting to true[/yellow]")
            return True
            
        except Exception as e:
            console.print(f"[yellow]Warning: Error evaluating expression '{expression}': {e}[/yellow]")
            return True
...
def prompt_password(field: FieldSchema, default: Any = None) -> str:
    ...
    if field.generate and not default:
        if Confirm.ask(f"Generate secure password for {field.label}?", default=True, console=console):
...
def prompt_textarea(field: FieldSchema, default: Any = None) -> str:
    ...
    console.print("[dim]Enter text (press Ctrl+D when finished, or type 'END' on a new line):[/dim]")
    ...
    while True:
        try:
            ...
            if not re.match(r'^[A-Z][A-Z0-9_]*$', key):
                console.print("[red]Error: Key must be uppercase letters, numbers, and underscores[/red]")
                continue
