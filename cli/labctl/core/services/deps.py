...
class DependencyGraph:
    ...
    def validate_dependencies(self) -> List[str]:
        ...
        for service_id, deps in self._graph.items():
            ...
            if missing:
                errors.append(f"Service '{service_id}' has missing dependencies: {', '.join(sorted(missing))}")
    ...
    def get_dependency_tree(self, service_id: str, max_depth: int = 10) -> Dict:
        ...
        def build_tree(svc_id: str, depth: int, visited: Set[str]) -> Dict:
            if depth >= max_depth or svc_id in visited:
                return {"id": svc_id, "name": self.schemas[svc_id].name, "truncated": depth >= max_depth}
    ...
    def suggest_removal_order(self, services_to_remove: List[str]) -> List[List[str]]:
        ...
        Returns:
            List of lists, where each inner list contains services that can be removed together
        """
        ...
        while remaining:
            ...
            for service in remaining:
                ...
                updated_external_deps = self.get_dependencies(service) - removal_set | set(removable)
                in_degree[service] = len(updated_external_deps & set(self.schemas.keys()) - removal_set)
