...
from .config import BaseServiceConfig, LabConfig
...
class ConfigurationValidator:
    ...
    def _validate_core_config(self):
        ...
        if not self._is_valid_domain(core.domain):
            self.errors.append(
                ValidationError(
                    f"Invalid domain format: '{core.domain}'. "
                    "Expected format: example.com or subdomain.example.com",
                    field="domain",
                )
            )
            ...
            self.warnings.append(
                ValidationWarning(
                    "Using .local domain in production profile. "
                    "Consider using a real domain for production deployment.",
                    field="domain",
                )
            )
        ...
        if not self._is_valid_email(core.email):
            self.errors.append(
                ValidationError(f"Invalid email format: '{core.email}'", field="email")
            )
        ...
        if hasattr(core, "timezone") and core.timezone:
            if not self._is_valid_timezone(core.timezone):
                self.warnings.append(
                    ValidationWarning(
                        f"Timezone '{core.timezone}' may not be valid. "
                        "Use formats like 'UTC', 'America/New_York'",
                        field="timezone",
                    )
                )
    ...
    def _validate_service(self, service_id: str, service_config: BaseServiceConfig):
        ...
        if hasattr(service_config, "port") and service_config.port:
            if not self._is_valid_port(service_config.port):
                ...
                self.warnings.append(
                    ValidationWarning(
                        f"Using privileged port {service_config.port} in development. "
    ...
    def _validate_traefik(self, config):
        ...
        if hasattr(config, "acme_environment"):
            if config.acme_environment not in ["staging", "production"]:
                self.errors.append(
                    ValidationError(
                        f"Invalid ACME environment: '{config.acme_environment}'. "
    ...
    def _validate_postgresql(self, config):
        ...
        if hasattr(config, "max_connections") and config.max_connections:
            if config.max_connections < 10 or config.max_connections > 1000:
                self.warnings.append(
                    ValidationWarning(
                        f"PostgreSQL max_connections ({config.max_connections}) "
                        "outside recommended range 10-1000",
        ...
        if hasattr(config, "shared_buffers") and config.shared_buffers:
            if not re.match(r"^\d+(MB|GB)$", config.shared_buffers):
                self.errors.append(
                    ValidationError(
                        f"Invalid shared_buffers format: '{config.shared_buffers}'. "
                        "Use format like '128MB' or '1GB'",
    ...
    def _validate_redis(self, config):
        ...
        if hasattr(config, "persistence"):
            if config.persistence not in ["rdb", "aof", "both", "none"]:
                self.errors.append(
                    ValidationError(
                        f"Invalid Redis persistence mode: '{config.persistence}'. "
                        "Must be 'rdb', 'aof', 'both', or 'none'",
                        field="persistence",
                        service="redis",
                    )
                )
        ...
        if hasattr(config, "maxmemory") and config.maxmemory:
            if not re.match(r"^\d+(mb|gb)$", config.maxmemory.lower()):
                self.errors.append(
                    ValidationError(
                        f"Invalid Redis maxmemory format: '{config.maxmemory}'. "
                        "Use format like '256mb' or '1gb'",
                        field="maxmemory",
                        service="redis",
                    )
                )
    ...
    def _validate_nextcloud(self, config):
        ...
        if hasattr(config, "upload_max_filesize") and config.upload_max_filesize:
            if not re.match(r"^\d+(M|G)$", config.upload_max_filesize):
                self.errors.append(
                    ValidationError(
                        f"Invalid upload_max_filesize format: "
                        f"'{config.upload_max_filesize}'. "
                        "Use format like '2G' or '512M'",
        ...
        if hasattr(config, "upstream_dns") and config.upstream_dns:
            for dns in config.upstream_dns:
                if not self._is_valid_ip_or_domain(dns):
                    self.errors.append(
                        ValidationError(
                            f"Invalid upstream DNS server: '{dns}'. "
                            "Must be IP address or domain",
                            field="upstream_dns",
                            service="pihole",
                        )
                    )
    ...
    def _validate_vaultwarden(self, config):
        ...
        if hasattr(config, "smtp_host") and config.smtp_host:
            if not self._is_valid_domain(config.smtp_host) and not self._is_valid_ip(
                config.smtp_host
            ):
                self.errors.append(
                    ValidationError(
        ...
        if hasattr(config, "smtp_port") and config.smtp_port:
            ...
            if config.smtp_port not in common_smtp_ports:
                self.warnings.append(
                    ValidationWarning(
                        f"Unusual SMTP port {config.smtp_port}. "
                        f"Common ports: {common_smtp_ports}",
                        field="smtp_port",
                        service="vaultwarden",
                    )
                )
        ...
        if hasattr(config, "ldap_enabled") and config.ldap_enabled:
            if hasattr(config, "ldap_host") and not config.ldap_host:
                ...
                if not self._is_valid_ldap_url(config.ldap_host):
                    self.errors.append(
                        ValidationError(
                            f"Invalid LDAP URL: '{config.ldap_host}'. "
                            "Expected format: ldap://host:port or ldaps://host:port",
                            field="ldap_host",
                            service="gitlab",
                        )
                    )
        ...
        for service_id, required_deps in dependencies.items():
            if service_id in service_ids:
                ...
                if missing_deps:
                    self.errors.append(
                        ValidationError(
                            f"Service '{service_id}' requires dependencies: "
                            f"{', '.join(missing_deps)}",
                            service=service_id,
                        )
                    )
    ...
    def _check_resource_requirements(self, services: Dict[str, BaseServiceConfig]):
        ...
        if len(enabled_heavy) > 2:
            self.warnings.append(
                ValidationWarning(
                    f"Multiple resource-intensive services enabled: "
                    f"{', '.join(enabled_heavy)}. "
...
def validate_configuration(
    config: LabConfig, show_results: bool = True
...
def display_validation_results(
    ...
    if errors:
        ...
        for error in errors:
            error_table.add_row(error.service or "-", error.field or "-", error.message)
    ...
    if errors:
        console.print(
            f"\n[red]❌ Configuration has {error_count} error(s) "
            "that must be fixed before deployment.[/red]"
        )
    elif warnings:
        console.print(
            f"\n[yellow]⚠️  Configuration has {warning_count} warning(s). "
            "Review before production use.[/yellow]"
        )
...
if __name__ == "__main__":
    ...
    test_config = LabConfig(
        core=CoreConfig(domain="example.com", email="admin@example.com"), services={}
    )
